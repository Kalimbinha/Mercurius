...
def Mercurius(
    ...
    *,
    ...
    Parâmetros mínimos:
    - app: instância FastAPI (ou objeto que tenha include_router)
    - model: classe SQLAlchemy (ORM)
    - read_schema: Pydantic model usado como response_model
    - db_session: callable dependency que retorna uma sqlalchemy.orm.Session

    Opcionalmente você pode fornecer `create_schema` e `update_schema` (Pydantic),
    nome da chave primária (`pk_name`) e quais operações incluir.
    """
